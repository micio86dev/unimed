...
const hardestBars = computed(() =>
  (data.value?.hardest_subjects ?? []).map((s) => ({
    ...
    value: s.accuracy,
    color: s.color,
    meta: `${s.answered} ${t('admin.answered')}`,
  })),
)
...
<template>
  <div class="space-y-8">
    ...
    <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
      ...
      <template v-else>
        <StatCard :label="$t('admin.totalStudents')" :value="data?.kpis.total_students ?? 0" :icon="Users" tone="primary" />
        <StatCard :label="$t('admin.active30d')" :value="data?.kpis.active_users ?? 0" :icon="UserCheck" tone="success" :hint="$t('admin.activeHint')" />
        <StatCard :label="$t('admin.completedQuizzes')" :value="data?.kpis.completed_quizzes ?? 0" :icon="ListChecks" tone="primary" />
        <StatCard :label="$t('admin.avgScore')" :value="formatPercent(data?.kpis.average_score, 1)" :icon="Target" tone="warning" />
      </template>
    </div>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-5">
      <Card class="lg:col-span-3">
        <CardHeader>
          <CardTitle>{{ $t('admin.hardestSubjects') }}</CardTitle>
          <CardDescription>{{ $t('admin.hardestSubjectsDesc') }}</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card class="lg:col-span-2">
        <CardHeader>
          <CardTitle>{{ $t('admin.activity14d') }}</CardTitle>
          <CardDescription>{{ $t('admin.activity14dDesc') }}</CardDescription>
        </CardHeader>
        <CardContent>
          <ChartsLineChart v-if="trendValues.length" :values="trendValues" :max="trendMax" :height="120" />
          <div class="mt-3 flex items-center justify-between text-xs text-muted-foreground">
            <span>{{ trend[0]?.date }}</span>
            <span class="flex items-center gap-1"><Activity class="size-3.5" /> {{ trendValues.reduce((a, b) => a + b, 0) }} {{ $t('admin.total') }}</span>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
      <Card hover class="cursor-pointer" @click="navigateTo('/admin/questions')">
        <CardContent class="flex items-center gap-4 p-5">
          ...
          <div>
            <p class="text-sm text-muted-foreground">{{ $t('admin.questionBank') }}</p>
      ...
      <Card hover class="cursor-pointer" @click="navigateTo('/admin/quizzes')">
        <CardContent class="flex items-center gap-4 p-5">
          ...
          <div>
            <p class="text-sm text-muted-foreground">{{ $t('admin.quizzes') }}</p>
